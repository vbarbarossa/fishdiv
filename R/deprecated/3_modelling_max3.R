...
res <- foreach(Qvar = Q_magnitude) %do% {
  ...
  write.csv(t_res,paste0('tabs/dredge_coefficients_ranslope_',response_selection,'_',Qvar,'_max3.csv'),row.names = F)
...
res_filtered <- foreach(i = seq_along(res)) %do% {
  ...
  write.csv(dfilt,paste0('tabs/dredge_coefficients_ranslope_',response_selection,'_',names(res)[i],'_FILTERED_max3.csv'),row.names = F)
