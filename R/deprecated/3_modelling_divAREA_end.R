...
write.csv(tab_ranges,'tabs/covariates_range_values_divAREA_end.csv',row.names = T)
...
tab %>%
  select(-ID,-BAS, -HFP1993, -POP, -DAMS, -URB, -CROP_PRES) %>%
  keep(is.numeric) %>% 
  gather() %>% 
  ggplot(aes(value)) +
...
tab.t %>%
  select(-HFP1993, -POP, -DAMS, -URB, -CROP_PRES) %>%
  gather() %>% 
  ggplot(aes(value)) +
  facet_wrap(~ key, scales = "free") +
  geom_histogram() +
...
cm <- cor(tab.t %>% select(-starts_with('SR'),-HFP1993, -POP, -DAMS, -URB, -CROP_PRES), use = "pairwise.complete.obs")
# and visualize
corrplot::corrplot(cm, method = 'number', type = 'lower', number.cex = 0.8)

# and save
jpeg('figs/corrplot_covariates_transformed_divAREA_end.jpg',width = 200, height = 200, res = 600, units = 'mm')
corrplot::corrplot(cm, method = 'number', type = 'lower',number.cex = 0.8)
dev.off()
...
write.csv(tab,'tabs/input_tab_divAREA_end.csv',row.names=F)
write.csv(tab.t,'tabs/input_tab_transformed_divAREA_end.csv',row.names=F)
