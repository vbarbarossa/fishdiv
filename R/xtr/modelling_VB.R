...
tab_d <- tab %>%
  ...
  select(-QUALITY,-PREC_HIST,-TEMP_HIST,-CROP_PAST,-POP,-NO_DAMS)
...
#' looks as if CROP_DELTA has been transformed, but is not really needed 

# check
BN$CROP_DELTA

# I would not use a log-trans here. Very small values are causing inflated log-transformed values. 
# Let's use the original values and correct the BN list
v2 <- scale(tab_d$CROP_DELTA)
BN$CROP_DELTA$BN <- NULL
BN$CROP_DELTA$center <- attr(v2,"scaled:center")
BN$CROP_DELTA$scale <- attr(v2,"scaled:scale")

tab_d.t$CROP_DELTA <- as.numeric(v2)
...
corvif(tab_d.t %>% select(-starts_with('SR'),-Q_MEAN,-Q_DOYCT))

corvif(tab_d.t %>% select(-starts_with('SR'),-Q_MAX,-Q_DOYCT))

corvif(tab_d.t %>% select(-starts_with('SR'),-Q_MIN,-Q_DOYCT))

corvif(tab_d.t %>% select(-starts_with('SR'),-Q_MEAN, -Q_MAX, -Q_DOYCT))

corvif(tab_d.t %>% select(-starts_with('SR'),-Q_MAX, -Q_MIN, -Q_DOYCT))

corvif(tab_d.t %>% select(-starts_with('SR'),-Q_MIN, -Q_MEAN, -Q_DOYCT))

corvif(tab_d.t %>% select(-starts_with('SR'),-Q_MEAN,-Q_DOYCT, -TI))

corvif(tab_d.t %>% select(-starts_with('SR'),-Q_MEAN,-Q_DOYCT, -DRAINAGE))

corvif(tab_d.t %>% select(-starts_with('SR'),-Q_MEAN,-Q_DOYCT, -SLOPE))

#' * Q_MAX and Q_MEAN cannot stay together while it looks like the other combinations work fairly well. Max and min are the least correlated, but they do not represent annual average flow. The alternative is to have one model per flow variables.
#'  
...
# define variables
covariates_selection <- c(tab_d.t %>% select(-BAS,-starts_with('SR'), -starts_with('Q_M'), -Q_DOYCT, -DRAINAGE, -AREA) %>% colnames)
covariates_selection

response_selection = 'SR_tot_dens'
random_term <- 'BAS'
...
res <- foreach(Qvar = Q_magnitude) %do% {
  ...
  # GGally::ggpairs(df %>% select(P_MWh, storage, height, qav, qcv),progress = FALSE)
